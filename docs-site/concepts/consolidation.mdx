...
<Info>
**v0.14+ note**: `belief revise` uses contradiction detection, which is gated by `use_legacy_heuristics`. With the flag set to `false` and no model bound, this returns `{reinforced: [], contradicted: [], suggested_new: []}` (safe defaults) rather than unreliable keyword heuristics. See [Pipeline Glossary](/concepts/pipeline-glossary#the-use_legacy_heuristics-flag) for details.
</Info>
...
<Steps>
  ...
  <Step title="Check for Contradictions">
    ...
    Contradiction detection is gated by `use_legacy_heuristics` (see [Pipeline Glossary](/concepts/pipeline-glossary#the-use_legacy_heuristics-flag)). With `use_legacy_heuristics=false` and no model, this returns an empty list rather than unreliable keyword matches.
  </Step>
</Steps>
