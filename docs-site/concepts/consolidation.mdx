...
<Info>
**v0.14+**: Contradiction detection is controlled by `use_legacy_heuristics`. When set to `false` without a model, `belief revise` returns empty results (safe defaults) instead of keyword heuristics. See [Pipeline Glossary](/concepts/pipeline-glossary#the-use_legacy_heuristics-flag).
</Info>
...
<Steps>
  ...
  <Step title="Check for Contradictions">
    ...
    With `use_legacy_heuristics=false` and no model, this returns an empty list instead of keyword matches. See [Pipeline Glossary](/concepts/pipeline-glossary#the-use_legacy_heuristics-flag).
  </Step>
</Steps>
