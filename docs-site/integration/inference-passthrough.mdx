...
If the client doesn't support sampling, Kernle operates in capture-only mode — memory operations (episodes, notes, beliefs) still work, but inference-dependent features like emotion detection and contradiction finding are skipped.
...
Kernle works without a model. Memory capture (`raw()`, `episode()`, `note()`, `belief()`) always works. Inference-dependent features return safe defaults:

| Feature | Without Model |
|---------|--------------|
| `raw()` | Works normally |
| `episode()` | Works normally |
| `note()`, `belief()`, `value()` | Works normally |
